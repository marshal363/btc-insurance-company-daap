...
| CV-PR-224 | Implement `distributePolicyPremium(policyId)` internal action/helper in `convex/policyRegistry.ts`. Triggered after policy expiration and `premium-distribution-initiated` event processing. Simulates backend initiating on-chain distribution (mocking BI calls).                | 6          | ⬜     | CV-PR-218, CV-PR-223                  |          |
