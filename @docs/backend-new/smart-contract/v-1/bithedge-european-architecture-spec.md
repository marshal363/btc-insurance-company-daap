...
```
┌─────────────┐         ┌─────────────────┐         ┌──────────────┐         ┌─────────────────┐
│             │         │ Off-Chain System│         │ Policy       │         │ Oracle, Params, │
│  Frontend   │         │ (e.g., Convex)  │         │ Registry     │         │ MathLib (Verify)│
│  Components │         │                 │         │              │         │ Liquidity Pool  │
└──────┬──────┘         └────────┬────────┘         └───────┬───────┘         └────────┬────────┘
       │                         │                         │                          │
       ...
       │                         │                         │     record             │
       │                         │                         │                          │
       │                         │                         │ 12. Request collateral │
       │                         │                         │     locking (LP Vault) │
       │                         │                         │ ───────────────────────► │
       │                         │                         │                          │
       │                         │                         │ 13. Collateral locked  │
       │                         │                         │      confirmation      │
       │                         │                         │ ◄─────────────────────── │
       │                         │                         │                          │
       │                         │                         │ 14. Record premium     │
...
0.  **Premium Calculation (Off-Chain)**: Before initiating the on-chain transaction, the user's desired policy parameters are sent to an off-chain system (like your Convex setup). This system calculates the premium using its sophisticated models and market data. The calculated premium and potentially some key input parameters (if needed for on-chain verification) are returned to the frontend.

1.  **Liquidity Check After Premium Verification**: Policy Registry first calls the Math Library to _verify_ the submitted premium (calculated off-chain). If verification passes, it then checks available liquidity with the Liquidity Pool _before_ accepting the (now verified) premium from the user and locking collateral (steps 5-8).

2.  **Data passed to `create-protection-policy`**: The frontend will call `create-protection-policy` in the Policy Registry, passing the standard policy parameters _and_ the `submitted_premium` obtained from the off-chain calculation.

3.  **Data passed during premium verification** (Policy Registry -> Math Library):

    ```clarity
    ;; Policy Registry -> Math Library (or relevant verification contract)
    (contract-call? .math-library verify-premium
      submitted-premium         ;; The premium calculated off-chain
      protected-value
      protection-amount
      expiration-height
      policy-type
      risk-tier
      ;; (potentially other key inputs from off-chain calculation for verification)
    )
    ```

4.  **Data passed during liquidity check**:
...
5. **Data passed during collateral locking**:
...
6. **Provider allocation selection process** (step 12):
...
7. **Premium recording data** (step 15):
   ```clarity
   ;; Policy Registry -> Liquidity Pool
